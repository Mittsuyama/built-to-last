'use server';
import type { FinancialReportData } from '@/types/financial-report';

const YEAR_REPORT_DATE = "-12-31";
const CURRENT_YEAR = (new Date()).getFullYear();
const CURRENT_MONTH = (new Date()).getMonth();
const LATEST_YEAR = CURRENT_MONTH > 3 ? CURRENT_YEAR - 1 : CURRENT_YEAR - 2;
...
const fetchReport = async (params: FetchRerpotParams) => {
  ...
  const batchResponse = await Promise.all(Array.from({ length: Math.ceil(dates.length / batch) }).map(async (_, index) => {
    const search = new URLSearchParams({
      ...body,
      dates: dates
        .slice(index * batch, Math.min((index + 1) * batch, dates.length))
        .join(','),
    });
    const res = await fetch(
      `${hostname}${path}?${search.toString()}`,
      { cache: 'force-cache' },
    )
...
const bundleRequest = async (params: BundleRequestParams & Pick<FetchRerpotParams, 'hostname' | 'path' | 'years'>) => {
